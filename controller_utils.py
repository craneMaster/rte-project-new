...
def create_split_constraint_controllers(grid, num_controllers, partition, T, H, batt_cost, curt_change_cost, curt_net_cost,
    ...
    Args:
        ...
        line_terminal_cost (Double):
            Weight on squared deviation of terminal horizon line flows from the nominal construction
            baseline (fixed across cycles; not updated on handoff).
        batt_curt_terminal_cost (Double):
...
def get_next_action(grid, controller_list, disturbances, t, dQPTH_layer, verbose=False, with_cvxpy=False, update_state=False):
    ...
    Returns:
        ...
        mpc_terminal_line_cost (Torch scalar):
            Sum of MPC terminal line-flow costs across all agents for this solve.
    """
    ...
    scipy_G = []
    scipy_A = []
    mpc_terminal_line_cost = torch.tensor(0.0)
    ...
    try:
        for i in range(num_agents):
            ...
            mpc_terminal_line_cost = mpc_terminal_line_cost + controller.mpc_terminal_cost(bus_states, line_states)
        ...
        print(e)
    return pred_actions_curt, pred_actions_batt, pred_line_max_slacks, pred_line_min_slacks, pred_bus_slacks, mpc_terminal_line_cost
