...
def main(args):
    ...
    folder = os.path.join(ROOT_DIR, f"results/dec/")
    tag = build_tag(parser, args, bool_as_int=False, only_nondefaults=False)
    print(f"Results for this run will be saved at {folder}dec_{tag}.pt")
    ...
    if test_skew_mag == 0:
        ...
        for i in range(num_test_traj):
            ...
            ckpt_base = evals.evaluate_base_on_traj(grid, base_controller_list, dQPTH_layer, test_traj, T, H, batt_cost, curt_change_cost, curt_net_cost, bus_slack_cost, line_slack_cost)
            base_losses[i] = ckpt_base["total_losses"]
        ckpt[("base")] = base_losses
    else:
        ...
        for test_seed in tqdm(range(3)):
            ...
            for j in tqdm(range(num_test_traj)):
                ...
                ckpt_base = evals.evaluate_base_on_traj(grid, base_controller_list, dQPTH_layer, test_traj, T, H, batt_cost, curt_change_cost, curt_net_cost, bus_slack_cost, line_slack_cost)
                base_losses[j] = ckpt_base["total_losses"]
            ckpt[(test_seed, "base")] = base_losses
