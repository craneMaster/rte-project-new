...
def sparse_LSQ(A,b,lsq_solver="scipy",eps_abs=1e-5,eps_rel=1e-5,x0=None):
    '''
    ...
    if lsq_solver in qpsolvers.sparse_solvers:
        ...
        # col_norms_sq = A.power(2).sum(axis=0)          # 1 x n sparse matrix
        # col_norms = np.sqrt(col_norms_sq.A1)           # .A1 -> flat ndarray

        # # Handle zero columns: avoid division by zero
        # # For zero columns, just leave scaling factor as 1 (or whatever makes sense)
        # scale = np.ones_like(col_norms)
        # nonzero_cols = col_norms > 0
        # scale[nonzero_cols] = 1.0 / col_norms[nonzero_cols]

        # # 2. Form column scaling matrix D = diag(scale)
        # D = sp.sparse.diags(scale)                            # shape (n, n), sparse diagonal

        # # 3. Build scaled matrix A_tilde = A * D
        # A_tilde = A @ D

        x, _, _, r = sp.sparse.linalg.lsqr(A, b, atol=eps_rel, btol=eps_rel, x0=x0)[:4]
        x = np.expand_dims(x, axis=-1)
