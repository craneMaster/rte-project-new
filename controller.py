...
class Split_Constraint_Controller(Base_Controller):
    ...
    def __init__(self, grid, H, buses_in_area, init_line_max, init_line_min, batt_cost, curt_change_cost,
                 curt_net_cost, bus_slack_cost, line_slack_cost, eps_abs=1e-5, eps=1e-8):
        """
        Args:
            grid (Grid):
                Full grid that the agent runs on.
        ...
        self.F_vec = F_vec
        raw_q = 2 * scale_vec * true_cost_vec * scale_vec
        # print(f"scaled batt cost is {raw_q[0]}")
        # print(f"scaled curt net cost is {raw_q[2 * self.num_batt]}")
        # print(f"scaled curt action cost is {raw_q[self.action_curt_start_idx]}")
        # print(f"scaled line slack cost is {raw_q[self.line_max_slack_start_idx]}")
        # print(f"largest index is at {torch.argmax(raw_q)}")

        # print(f"line_state_start_idx {self.line_state_start_idx}, line_max_slack_start_idx {self.line_max_slack_start_idx}\n" + 
        #       f"line_min_slack_start_idx {self.line_min_slack_start_idx}, action_batt_start_idx {self.action_batt_start_idx}, action_curt_start_idx {self.action_curt_start_idx}")

        cost_scale = torch.max(torch.abs(raw_q))
        # print(f"batt: {raw_q[0]}, curt net: {raw_q[2*self.num_batt]}, curt action: {raw_q[self.action_curt_start_idx]}, line: {raw_q[self.line_max_slack_start_idx]}, cost_scale {cost_scale}")
        ridge_eps = 1e-8 * cost_scale
        # print(f"ridge_eps: {ridge_eps}")
        # print(f"ridge_eps: {ridge_eps}")
    ...
    def reset_params(self):
        ...
        self.line_max_change = nn.Parameter(self.init_line_max_change_buf.clone())
        self.line_min_change = nn.Parameter(self.init_line_min_change_buf.clone())
        # self.dQP_layer.reset_cache()

    
    def make_decision(self, disturbances, t, line_max_change=None, line_min_change=None, verbose=False, update_state=False):
        """
        Args:
            disturbances (Torch tensor of shape (H, num_buses)):
        ...
        Returns:
            ...
            line_min_slacks (Torch tensor of shape (H, grid.num_lines)):
                Next H slack values on line min.
        """
        if line_max_change == None:
            line_max_change = self.line_max_change
        if line_min_change == None:
            line_min_change = self.line_min_change
        ...
        h = torch.zeros(self.ineq_dim)
        for i in range(self.H):
            row_idx = i * self.bus_slack_dim
            h[row_idx:row_idx+self.bus_slack_dim] = self.H_limit

            row_idx = i * self.line_state_dim + self.H * self.bus_slack_dim
            line_max = torch.sum(line_max_change[0:t+i+1], dim=0)
            h[row_idx:row_idx+self.line_state_dim] = line_max

            row_idx = i * self.line_state_dim + self.H * (self.bus_slack_dim + self.line_state_dim)
            line_min = torch.sum(line_min_change[0:t+i+1], dim=0)
            h[row_idx:row_idx+self.line_state_dim] = -line_min

        # z = qpth.qp.QPFunction(eps=1e-12, verbose=verbose)(
        #     self.Q, self.q, self.G, h, self.A, b, self.Q_LU, self.S_LU, self.R
        # )
        z, _, _, _, _ = self.dQP_layer(self.sparse_Q, self.q, self.sparse_G, h, t, self.sparse_A, b)
        z = z.flatten()
        ...
        bus_slacks = torch.zeros(self.H, self.bus_slack_dim)
        for i in range(self.H):
            bus_slacks[i] = z[i*self.bus_slack_dim+self.bus_slack_start_idx:(i+1)*self.bus_slack_dim+self.bus_slack_start_idx]
        ...
        with torch.no_grad():
            for i in range(self.H):
                ...
                objective += self.curt_change_cost * torch.sum(actions_curt[i]**2)
                objective += self.curt_net_cost * torch.sum(bus_states[i][self.bus_state_curt_idx]**2)
                objective += self.slack_cost * torch.sum(bus_slacks[i]**2)
                objective += self.slack_cost * torch.sum(line_max_slacks[i]**2)
                objective += self.slack_cost * torch.sum(line_min_slacks[i]**2)
            objective += self.eps * torch.sum(z ** 2)
